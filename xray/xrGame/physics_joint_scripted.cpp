...
void cphysics_joint_scripted::script_register(lua_State *L)
{
	module(L)
		[
			...
			.def("get_max_force_and_velocity",			&cphysics_joint_scripted::GetMaxForceAndVelocity, policy_list<policy::out_value<2>, policy::out_value<3>>())
			.def("get_axis_angle",						&cphysics_joint_scripted::GetAxisAngle)
			.def("get_limits",							&cphysics_joint_scripted::GetLimits, policy_list<policy::out_value<2>, policy::out_value<3>>())
			.def("get_axis_dir",						&cphysics_joint_scripted::GetAxisDirDynamic, policy::out_value<3>())
			.def("get_anchor",							&cphysics_joint_scripted::GetAnchorDynamic, policy::out_value<2>())
