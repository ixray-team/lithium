   ...
   sh_name[len] = '\0';
	...
	if (FS.exist(file_name))
	{
		...
		if (file->length()>4)
		{
			...
			u32 const real_crc		= crc::crcFast((unsigned char*)file->pointer(), file->elapsed());
	...
	if (FAILED(_result))
	{
		...
		if (SUCCEEDED(_result))
		{
			...
			u32 const crc			= crc::crcFast((unsigned char*)pShaderBuf->GetBufferPointer(), pShaderBuf->GetBufferSize());
