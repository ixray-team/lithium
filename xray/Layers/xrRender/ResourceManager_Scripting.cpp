...
void	CResourceManager::LS_Load()
{
	...
	[
		...
		class_<adopt_sampler>("_sampler")
			...
			.def("texture",						&adopt_sampler::_texture		,policy::return_reference_to<1>())
			.def("project",						&adopt_sampler::_projective		,policy::return_reference_to<1>())
			.def("clamp",						&adopt_sampler::_clamp			,policy::return_reference_to<1>())
			.def("wrap",						&adopt_sampler::_wrap			,policy::return_reference_to<1>())
			.def("mirror",						&adopt_sampler::_mirror			,policy::return_reference_to<1>())
			.def("f_anisotropic",				&adopt_sampler::_f_anisotropic	,policy::return_reference_to<1>())
			.def("f_trilinear",					&adopt_sampler::_f_trilinear	,policy::return_reference_to<1>())
			.def("f_bilinear",					&adopt_sampler::_f_bilinear		,policy::return_reference_to<1>())
			.def("f_linear",					&adopt_sampler::_f_linear		,policy::return_reference_to<1>())
			.def("f_none",						&adopt_sampler::_f_none			,policy::return_reference_to<1>())
			.def("fmin_none",					&adopt_sampler::_fmin_none		,policy::return_reference_to<1>())
			.def("fmin_point",					&adopt_sampler::_fmin_point		,policy::return_reference_to<1>())
			.def("fmin_linear",					&adopt_sampler::_fmin_linear	,policy::return_reference_to<1>())
			.def("fmin_aniso",					&adopt_sampler::_fmin_aniso		,policy::return_reference_to<1>())
			.def("fmip_none",					&adopt_sampler::_fmip_none		,policy::return_reference_to<1>())
			.def("fmip_point",					&adopt_sampler::_fmip_point		,policy::return_reference_to<1>())
			.def("fmip_linear",					&adopt_sampler::_fmip_linear	,policy::return_reference_to<1>())
			.def("fmag_none",					&adopt_sampler::_fmag_none		,policy::return_reference_to<1>())
			.def("fmag_point",					&adopt_sampler::_fmag_point		,policy::return_reference_to<1>())
			.def("fmag_linear",					&adopt_sampler::_fmag_linear	,policy::return_reference_to<1>()),

		class_<adopt_compiler>("_compiler")
			.def(								constructor<const adopt_compiler&>())
			.def("begin",						&adopt_compiler::_pass			,policy::return_reference_to<1>())
			.def("sorting",						&adopt_compiler::_options		,policy::return_reference_to<1>())
			.def("emissive",					&adopt_compiler::_o_emissive	,policy::return_reference_to<1>())
			.def("distort",						&adopt_compiler::_o_distort		,policy::return_reference_to<1>())
			.def("wmark",						&adopt_compiler::_o_wmark		,policy::return_reference_to<1>())
			.def("fog",							&adopt_compiler::_fog			,policy::return_reference_to<1>())
			.def("zb",							&adopt_compiler::_ZB			,policy::return_reference_to<1>())
			.def("blend",						&adopt_compiler::_blend			,policy::return_reference_to<1>())
			.def("aref",						&adopt_compiler::_aref			,policy::return_reference_to<1>())
			.def("color_write_enable",			&adopt_compiler::_color_write_enable, policy::return_reference_to<1>())
