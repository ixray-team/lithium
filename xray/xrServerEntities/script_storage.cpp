...
void CScriptStorage::reinit	()
{
	...
	// @Schtainhardt: gfind is renamed to gmatch after Lua 5.1 so fix it
	pcstr gfind_fix = "string[\"gfind\"] = string.gmatch";
	luaL_dostring(lua(), gfind_fix);
	
	// the same for mod
	pcstr mod_fix = "math[\"mod\"] = math.fmod";
	luaL_dostring(lua(), mod_fix);
	// << end
...
struct raii_guard : private onyx::noncopyable {
	int m_error_code;
