...
ObjectFactory::CLIENT_BASE_CLASS *CObjectItemScript::client_object	() const
{
	...
	try {
		...
		object = luabind::object_cast<ObjectFactory::CLIENT_BASE_CLASS*>(instance, luabind::policy::adopt<0>());
	}
	catch(...) {
		return	(0);
	}
...
ObjectFactory::SERVER_BASE_CLASS *CObjectItemScript::server_object	(LPCSTR section) const
{
	...
	try {
		try {
			...
			object = luabind::object_cast<ObjectFactory::SERVER_SCRIPT_BASE_CLASS*>(instance, luabind::policy::adopt<0>());
		}
